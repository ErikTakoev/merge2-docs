...
**Використовується в**: `MergeLightDragFeedback`

Візуальний ефект спалаху або підсвічування, який відтворюється на результуючій фішці після успішного злиття.
- **Особливості**:
  - Активація відбувається автоматично після успішного виконання злиття через систему фідбеку `MergeLightDragFeedback`.
  - Ефект запускається на результуючій фішці за допомогою методу `Activate(chip)`.
  - Оскільки попередній чіп знищується, а новий з'являється на його місці, ефект злиття допомагає зробити перехід візуально безшовним. Його `destroyAfterActivate` зазвичай дорівнює `true`, що дозволяє йому автоматично самоочиститися з поля після завершення анімації спалаху.
