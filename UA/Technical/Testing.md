...
#### CheckModifierPower_Evolution
- **Мета**: Перевіряє коректність очищення модифікаторів при еволюції (злитті) бустера, що впливає на роботу сусідніх генераторів.
- **Сценарій**:
  ...
  3. Генератор знаходиться в діапазоні першого бустера → `PowerBoosterModifiers.Count == 1`.
  4. Перетягує другий бустер на перший (drag з (4, 6) на (4, 0)) — це викликає **злиття/еволюцію** бустерів.
  5. **Ключова перевірка**: Після merge генератор повинен мати `PowerBoosterModifiers.Count == 0` (старий бустер знищений та очистив модифікатори).
  6. **Верифікація**: Чекає час `ChargingTime - timeToMove`. Генератор залишається UNCHARGED, оскільки модифікатор був знято при знищенні першого бустера.
- **Важливість**: Гарантує, що `PowerBoosterCellSubscriber.OnChipDestroy()` коректно видаляє всі модифікатори перед знищенням бустера, щоб не залишати "привидів" активних бустерів у генераторів.
