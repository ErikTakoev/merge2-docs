...
- **`IScenarioEventHandler`** -> `ScenarioEventHandler`
  ...
  - **Відповідальність**: Трансляція життєвого циклу чіпів та змін на полі (створення чіпа, знищення, зняття блокуючого ефекту, розблокування зони) у C# події. Виступає сполучною ланкою між ядром Merge та системою сценаріїв (трансляція у події Unity Visual Scripting EventBus винесена в окремий адаптер `VScriptingScenarioEventBridge` в збірці `MergeBase.VScripting`).
