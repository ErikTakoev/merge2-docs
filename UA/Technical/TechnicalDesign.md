...
- **`IChipCollections`** -> `ChipCollections`
  ...
  - **Відповідальність**: Збереження `AllChipsByData`, а також підтримка актуальності словників `MergeableChipsByData` (фішки, доступні як цілі для злиття), `FillableChipsByData` (контейнери, що очікують наповнення) та `MovableChipsByData` (рухомі фішки з незаблокованими клітинками). Оновлюється при створенні/знищенні фішок, а також через події зміни блокувань (`OnChipBlockingChanged`) та оновлення вимог контейнера (`OnContainerRequirementsChanged`). Надає методи `GetCollection(ChipCollectionType)` та `GetCollection(ChipData, ChipCollectionType)` для отримання відфільтрованих списків за `ChipCollectionType` enum (All, Mergeable, Fillable, Movable).
...
- **`IFreeCellFinder`** -> `FreeCellFinder`
  ...
  - **Відповідальність**: Знаходження найближчої вільної комірки (спіральний пошук) для спавну або переміщення чіпів через `FindNearestFreeCell`. Підтримує режим `onlyAround` для обмеження пошуку тільки сусідніми комірками, а також атомарний батч-пошук вільних комірок для декількох чіпів через `FindFreeCellsForChips` без додаткових GC-алокацій.
...
- **`IScenarioEventHandler`** -> `ScenarioEventHandler`
  ...
  - **Відповідальність**: Трансляція життєвого циклу чіпів та змін на полі (створення чіпа, знищення, зняття блокуючого ефекту, розблокування зони, відсутність вільного місця `OnNotEnoughSpace`) у C# події. Виступає сполучною ланкою між ядром Merge та системою сценаріїв (трансляція у події Unity Visual Scripting EventBus винесена в окремий адаптер `VScriptingScenarioEventBridge` в збірці `MergeBase.VScripting`).
