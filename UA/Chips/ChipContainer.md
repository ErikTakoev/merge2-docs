...
- **Ефекти**:
  - **[ChipContainerEffect](../Visuals/Effects.md#4-chip-container-вимоги-контейнера)**: Спеціальний ефект, який візуалізує необхідні предмети ("бабли") над контейнером.
...
**Власні властивості**:
- **`containers`**: Словник (`Dictionary<ContainerInfo, int>`), що відслідковує поточний прогрес заповнення контейнера.
  - **Ключ (`ContainerInfo`)**: Визначення вимоги (тип предмету, ID, кількість).
  - **Значення (`int`)**: Поточна кількість зібраних предметів для цієї вимоги.
  - **Поведінка**: Коли вимога виконана повністю (значення досягає `ContainerInfo.Count`), запис видаляється зі словника.
  - **Завершення**: Порожній словник означає, що контейнер повністю заповнений.

**Ініціалізація**: Створюється в `ChipContainer.Init()` на основі `ChipContainerData.containers`, встановлюючи початкове значення 0 для кожної вимоги.
...
Спеціалізований ефект для візуалізації вимог контейнера. Керує Panel над фішкою, в якій відображаються потрібні предмети.
