...
## Методи та Логіка
- **`InitEffects()` (Override)**: Перевизначає базовий метод для ініціалізації генератор-специфічних ефектів. Спочатку викликає `base.InitEffects()` для інстанціювання стандартних ефектів, потім додає `ChargedEffect` та `RechargeEffect`, якщо вони призначені в інспекторі. Це дозволяє генератору розширити поведінку базового класу без дублювання коду.
