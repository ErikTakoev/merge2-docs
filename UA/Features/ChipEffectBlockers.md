...
Блокування в Merge2 — це стан фішки, який забороняє певні ігрові дії (переміщення, злиття, генерацію тощо). Найчастіше блокування накладається через **Extra-ефекти** (ланцюги, коробки), які мають власні правила руйнування.
...
- **`CanReceiveModifiers`**: Дозвіл прийому модифікаторів (для `ChipGenerator`, `ChipPowerBooster`).
...
- **`EffectDestroyingSettings`** (`ScriptableObject`): Налаштування руйнування ефекту.
  - `NeighboringMergeCountToDestroy` (float): Поріг кількості сусідніх злиттів для руйнування.
  - `NeighboringMergeTriggerForEffect` (string): Префікс тригеру для прогресивної анімації.
...
### Data (FieldData)
Стан блокування зберігається у структурі `FieldChipData` всередині `FieldData`:
...
- **Слот модифікаторів**: Якщо `CanReceiveModifiers = false`, фішка ігнорує підсилення від бустерів (Power Boosters).
