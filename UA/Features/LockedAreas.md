...
- **Реєстрація ефектів** (`RegisterEffect`): Приймає реєстрацію від компонентів `LockedAreaEffect` та одразу викликає `Activate()` або `Deactivate()` залежно від поточного стану. Якщо вказаний `LockedAreaId` ефекту не існує у конфігурації поля (`FieldData.LockedAreas`), ефект автоматично вимикається (`effect.gameObject.SetActive(false)`).
